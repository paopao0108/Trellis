...
public class GameView : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler
{
    ...
    private Vector3 largeCirclePosInPanel; // 
    private int curRow;
    private int curCol;
    ...
    public void OnDrag(PointerEventData eventData)
    {
        if (isOnUI && circleCount > 0)
        {
            ...
            Vector3 mouseOffset = new Vector3(newMousePos.x - mousePos.x, newMousePos.y - mousePos.y, 0); // 拖动过程中鼠标位置的偏移量
                                                                                                          //largeCircle.position = curCirclePos + mouseOffset;
            ...
            largeCirclePosInPanel = new Vector3(curCircleClone.position.x - zeroPoint.Pos.x, curCircleClone.position.y - zeroPoint.Pos.y, 0);
            //Debug.Log("鼠标相对于格子的位置： " + offsetMouseAndGrid);
            //Debug.Log("圆圈相对于格子面板的位置： " + largeCirclePosInPanel);

            curRow = Mathf.RoundToInt(largeCirclePosInPanel.x / GridCell.CellSize);
            curCol = Mathf.RoundToInt(largeCirclePosInPanel.y / GridCell.CellSize);
